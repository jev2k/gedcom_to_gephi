...
class Individual(Element):
    ...
    def parents(self):
        ...
        if 'FAMC' in self:
            ...
            # print("family as child id",family_as_child_id)
    ...
    def name(self):
        ...
        Returns a tuple of (firstname, lastname). If firstname or lastname
        isn't in the file, then None is returned.
        :returns: (firstname, lastname)
        """
        preferred_name = "startnone"
        
        #let's find the name tags
        if not self['NAME']:
            if not self['GIVN']:
                if not self['SURN']:
                    print('\n*******',self,'\n')
                    name_tag = ["No_Name"]
                    first = "first name(self)"
                    last = "last name(self)"
                    return first,last
                else:
                    name_tag = self['SURN']
        ...
        # if name_tag[0]=="":
        #     print("\n\nfound an empty name tag",name_tag[0])
        # else:
        #     print("\n here's a good name tag",name_tag)
#~~~~~~~~~~
## NOW GET THE PREFERRED NAME
        if isinstance(name_tag, list):
            # We have more than one name, get the preferred name
            # Don't assume it's the first
            # for eachname in name_tag:
            try:
                print("\nname tag is a list",name_tag['NAME'])
            except:
                abc = 0
            # print("full name tag\n",name_tag)
            # print("nametag[0]\n",name_tag[0])
            preferred_name = name_tag[0]['NAME']

        else:
            # print("\n Error in init__py... name tag not a list\n name_tag=",name_tag)
            # We've only one name
            preferred_name = self['NAME'].value
            if preferred_name!=None:
                preferred_name = preferred_name.replace("/","")
            # print("we got a name?",preferred_name)
            
        if type(name_tag) == 'str':
            preferred_name = name_tag.replace("/","")
        
        elif preferred_name == ["No Name","No Name"] or preferred_name == None or preferred_name == "":
            first = None
            last = None
        # else:
        #     if preferred_name != []:
        ...
        if not preferred_name:
            first = "first None"
            last = "last None"
        # elif preferred_name.value in ('', None):
            try:
                if not self['GIVN']:
                    first = None
                else:
            ...
            # print('\npref name =',preferred_name)
            try:
                vals = preferred_name.value.split(" ")
            except:
                vals = preferred_name.split(" ")
            # print("vals = ",vals)
            assert len(vals) > 0
            if len(vals) == 1:
                # Only first name
                ...
                first = str(first.strip()) + str(" ") + str(mid.strip())
                last = last.strip()
                
            elif len(vals) ==4:
                first,mid1,mid2,last = vals
                first = str(first.strip()) + str(" ") + str(mid1.strip()) + str(" ") + str(mid2.strip())
                last = last.strip()
                
            elif len(vals) ==5:
                first,mid1,mid2,last1,last2 = vals
                first = str(first.strip()) + str(" ") + str(mid1.strip()) + str(" ") + str(mid2.strip())
                last = last1.strip() + " " + last2.strip()
                
            else:
                first = preferred_name
                last = ""
...
def __parse(lines_iter):
    ...
    for linenum, line in enumerate(lines_iter):
        ...
        if level == 0:
            ...
            level_to_obj = dict((lev, obj) for lev, obj in level_to_obj.items() if lev < level)
            # print("level_to_obj & level",level_to_obj, level)
            try: 
                parent = level_to_obj[level - 1]
            except:
                # print("oopsies on Parent")
                parent = None
